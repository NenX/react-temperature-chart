...
import { COLS_OF_RIGHT, HEIGHT_OF_BLOCK, HW_OF_GRID, WIDTH_OF_BLOCK, WIDTH_OF_LEFT } from '../constant';
import { IPointData, TPointArr } from '../types';


interface IDrawScaleConfig {
    markName: string
    x: string | number
    beginKD: number
    endKD: number
    ...
    stepSide: boolean
    valueShow: boolean
    isShowdemarcate: boolean
    martchs?: string
    svgId?: string
    isShowLittle?: boolean

    stepValue?: any
    demarcateId?: string
}
export function drawSvgScale(oJson: IDrawScaleConfig) {
    let scaleArray = [];
    ...
    let td_strokeWidth = 1;
    let disPX = (trRowEnd - trRowBegin) * HW_OF_GRID / ((endKD - beginKD) / disKD); //一刻度需要多少像素px
    let endY = trRowEnd * HW_OF_GRID
    x = (typeof (x) === "string") ? WIDTH_OF_LEFT * parseFloat(x) : x;
    if (martchs !== undefined) {  //华氏温度和摄氏温度换算时需要
        ...
        let y1 = tdBegin * HW_OF_GRID;
        disPX = (tdBegin - tdEnd) * HW_OF_GRID / ((scale2 - scale1) / disKD);
        if (scale1 > beginKD) {
    ...
    //当需要画标定线时
    if (isShowdemarcate) {
        let y1 = trRowEnd * HW_OF_GRID + disPX;
        let y2 = trRowBegin * HW_OF_GRID - 10
        scaleArray.push(<line key={0} id={demarcateId} x1={x} y1={y1} x2={x} y2={y2}
            style={{ stroke: '#3c3c3c', strokeWidth: td_strokeWidth }} />);
    }
    for (let i = 0; i <= ((endKD - beginKD) / disKD); i++) {
        ...
        if (isShowLittle) {
            if (i % 5 === 0) {
                scaleArray.push(<line key={i + 1} max={beginKD} min={endKD} type={markName + '_line'}
                    ...
                    style={{ stroke: '#3c3c3c', strokeWidth: td_strokeWidth, display: 'none' }} />);
            } else {
                scaleArray.push(<line key={i + 1} max={beginKD} min={endKD} type={markName + '_line'}
                    x1={(stepSide ? ((x) - width) : (x + width))}
        ...
        scaleArray.push(
            <text key={(i + 1) + "markName"} type={markName + '_line'}
                style={{ display: (valueShow ? "" : "none") }}
                x={(stepSide ? ((x) - (width + len * 8)) : (x + (width + 2)))}
                y={(endY - (disPX * i) + 4.5).toFixed(2)} fill='#3c3c3c'
            // value={(i * disKD + beginKD)}
            >{i * disKD + beginKD}</text>
        );
    }
    let b = 0;
 ...
 * @return
 * */
export function parseMapData(mapData: any) {
    let titleArray = [];
    let valueArray = [];
    ...
    //排序，name长的排在前面
    //使用快速排序
    function bubbleSort(arr: any, aboutarr: any) {
 ...
 */
export function getNewDate(dateStr: any, dis: any) {
...
export function drawEvent(eventArray: TPointArr, curDate: string) {
    ...
    let height = 15;
    //按时间分组，显示在同一时间段
    let timeArray: { [x: string]: TPointArr } = {};
    let otherArray = [];
    for (let i = 0; i < eventArray.length; i++) {
        ...
        if (value !== '开呼吸机' && value !== '关呼吸机' && value !== '不升') {
            if (timeArray[dataTime] === undefined) {
                timeArray[dataTime] = [];
                timeArray[dataTime].push(eventArray[i]);
            } else {
                timeArray[dataTime].push(eventArray[i]);
            }
        } else {
    ...
    for (let time in timeArray) {
        ...
        let xPx = getDisDays(curDate, time) * WIDTH_OF_BLOCK - 5.5;
        let show = "";
        let k = 0;
 ...
 * */
export function drawHzhx(hzhxData: any, curDate: any) {
    let svgW = 15 * 42;
    let svgH = 15 * 55;
    let tdW = 15; //td的宽度
    let tdH = 15;//td的高度
    ...
    for (let i = 0; i < hzhxData.length; i++) {
        ...
        hxArray.push(<text key={xPx + y} fill={'blue'} x={xPx - 6} y={y} >{value}</text>)
    }
    return hxArray;
}
 ...
 * @return
 */
export function parseRePoint(pointData: any, curDate: any) {
    let height = 15;
...
export function getPolygon(mbPoints: TPoint[][], xlPoints: TPoint[][]) {
    let polygonArray: TPoint[][] = [];
    let intersectionArray: TPoint[][] = []; //缓存交点数组
    for (let i = 0; i < mbPoints.length; i++) {
        intersectionArray[i] = [];
        let mbArray = mbPoints[i];
        let xlArray = xlPoints[i];
        console.log('mbArray', mbArray, xlArray)
    ...
    for (let i = 0; i < intersectionArray.length; i++) {
        ...
        for (let f = 0; f < interArray.length; f++) {
            if (interArray[f] && interArray[f + 1] !== undefined) {
                ...
                let morePointArray: TPoint[] = []; //保存相邻的两个 交点（心率）
                let lessPointArray: TPoint[] = []; //保存相邻的两个 交点（脉搏）
                for (let j = 0; j < mbPoint.length; j++) {
 ...
 */
export function splitGroup(leaveEvent: string[], point: TPointArr) {
    //没有请假事件
    if (leaveEvent.length === 0) {
        return [point];
    }
    //按请假事件分组 根据请假事件的数量，判断分组数量
    let groupArray: IPointData[][] = [];
    const groupRule: { min?: string, max?: string }[] = []; //缓存分组规则
 ...
 * @return {Boolean}
 */
function judgeIn(curTime: any, minTime: any, maxTime: any) {
...
export function getXY(point: IPointData, beginDate: string, originY: number, maxY: number, ySection: any, type = false) {
...
export function numToChinese(numArray: string) {
    let chineseArray = ["零", "一", "二", "三", "四", "五", "六", "七", "八", "九", "十"];

    let finalStr = "";
    if (numArray.length === 1) {
        finalStr = chineseArray[numArray[0]];
        return finalStr;
    } else {
        if (numArray[0] === "1") {
            if (numArray[1] === "0") {
                ...
                finalStr += "十" + chineseArray[numArray[1]];
            }
        } else {
            if (numArray[1] === "0") {
                finalStr += chineseArray[numArray[0]] + "十";
            } else {
                finalStr += chineseArray[numArray[0]] + "十" + chineseArray[numArray[1]];
            }
        }
    }
    return finalStr;
 ...
 * @return
 */
export function getDisDays(beginDate: any, endDate: any, beginHour = 2) {
 ...
 */
export function getParseFloat(number: any, sub: any) {
    return parseFloat(parseFloat(number).toFixed(sub));
}


export function drawSquare(type: string, xy: { x: any, y: any }, size: number, color: string, title: string, fill: boolean) {
    if (title === null || title === undefined) {
        return "";
    }
    let width = 1; //定义正方形的宽度
    return (<Tooltip key={"tt" + xy.y + xy.x} title={title} Content={forwardRef<SVGRectElement, {}>((p, ref) => <rect
        x={xy.x - size}
        y={xy.y - size}
        width={size * 2}
        height={size * 2}
...
export function drawCircle(type: string, xy: { x: any, y: any }, r: number, color: string, title: string, fill: boolean) {

    let width = 2;	//定义圆环的宽度
    let circleHtml;
    if (fill) { //实心
        circleHtml = <Tooltip key={type + xy.y + xy.x} title={title} Content={forwardRef<SVGCircleElement, {}>((p, ref) =>
            <circle ref={ref} cx={xy.x} cy={xy.y} r={r} strokeWidth={width} stroke={color} fill={color} />)
        }>
        </Tooltip>;
    } else { //空心
        circleHtml = <Tooltip key={type + xy.y + xy.x} title={title} Content={forwardRef<SVGCircleElement, {}>((p, ref) =>
            <circle ref={ref} key={type + xy.y + xy.x} cx={xy.x} cy={xy.y} r={r} strokeWidth={width} stroke={color} fill='#fff' />)
        }>
...
export function drawCross(type: string, xy: { x: any, y: any }, halfSize: number, color: string, title: string) {
    let html = [];

    let x = xy.x;
    let y = xy.y;
    html.push(
        <Tooltip key={0} title={title} Content={forwardRef<SVGLineElement, {}>((p, ref) =>
            <line ref={ref} type={type} cx={x} cy={y} x1={x - halfSize} y1={y - halfSize} x2={x + halfSize}
    ...
    html.push(
        <Tooltip key={1} title={title} Content={forwardRef<SVGLineElement, {}>((p, ref) =>
            <line ref={ref} type={type} x1={x + halfSize} y1={y - halfSize} x2={x - halfSize} y2={y + halfSize}
...
export function drawFiveStar(type: string, xy: { x: any, y: any }, size: number, color: string) {
...
export function drawArrow(xy: { x: any, y: any }, length: number, type: boolean, color: string) {
    let x = xy.x;
    let y1 = xy.y - length;
    let y2 = xy.y - 3;
    if (type) {
...
export function showPointTuglie() {
    function showText(x: number, y: number, text: string) {
...
function segmentsIntr(line1: any, line2: any) {
    let a1 = line1.a;
    let a2 = line2.a;
    let b1 = line1.b;
    let b2 = line2.b;
...
export function drawBlueLine(polygonArray: any) {
    let htmlArray = [];
...
function drawPolygon(points: any, color: any, key = '') {
    if (points.length === 0) {
        return {
            "html": '',
            "linear": '',
...
function getDoundary(point: any) {
    let maxX = +point[0].x;
    let minX = +point[0].x;
    let maxY = +point[0].y;
    let minY = +point[0].y;
